...
export default function SentimentDashboard({ initialSummary, initialSelected = [...SENTIMENT_LABELS] }: { initialSummary: SentimentDashboardSummary; initialSelected?: SentimentLabel[] }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1180px] px-5 py-8 sm:px-8 sm:py-10 lg:px-12 lg:py-14">
      ...
      <section aria-labelledby="chart-title" aria-busy={isPending} className="mt-5 rounded-[1.75rem] border border-navy/8 bg-white p-5 shadow-[0_16px_50px_rgba(26,31,61,0.05)] sm:p-8"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><p className="eyebrow text-[0.68rem]">seu ritmo</p><h2 id="chart-title" className="mt-2 text-2xl font-black tracking-[-0.05em] text-navy">Sentimentos por dia</h2></div><p className="text-xs font-bold text-navy/40 sm:text-right">{formatLongDate(summary.from)}<br className="sm:hidden" /> até {formatLongDate(summary.to)}</p></div>{summary.total ? <Chart summary={summary} selected={selected} /> : <div className="mt-8 grid min-h-[280px] place-items-center rounded-2xl border border-dashed border-navy/12 bg-warm/40 px-6 text-center"><div><p className="text-sm font-black text-navy/70">Ainda não há registros nesse recorte.</p><p className="mt-2 max-w-sm text-sm leading-6 text-navy/45">Tente ampliar o período ou registre como você está se sentindo hoje.</p></div></div>}<p className="mt-6 text-xs leading-5 text-navy/40" aria-live="polite">{notice || "Os registros são privados e servem apenas para você acompanhar seu momento."}</p></section>

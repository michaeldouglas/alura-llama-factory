...
import { NEUTRO_SUBAGENT_PROMPT } from "@/agent/prompts/subagents/neutro-prompt";
import { CONVERSATION_MODE_COPY, getConversationModeInstruction, type ConversationMode } from "@/lib/conversation";

export function createNeutroAgent(model: ChatOllama, mode: ConversationMode = "ouvir") {
  return createAgent({
    model,

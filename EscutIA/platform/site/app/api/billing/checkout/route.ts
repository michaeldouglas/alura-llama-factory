...
import { createCheckoutSession, findPriceByLookupKey, StripeApiError } from "@/lib/stripe";
...
export async function POST(request: Request) {
  ...
  const body = (await request.json().catch(() => null)) as { lookupKey?: unknown } | null;
  ...
  try {
    ...
    const checkout = await createCheckoutSession(baseParameters);

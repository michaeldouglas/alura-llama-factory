...
Como responsável pelo produto, quero que o harness reconheça
`EscutIA/platform/site/` como uma aplicação Next.js full-stack que contém o site
e a API, para que solicitações de aplicação não sejam confundidas com tarefas de
dataset ou treinamento.
...
- A implementação da plataforma tenta criar uma API irmã em `platform/api/` ou usar o `testar_modelo_lora.py` apesar do contrato JavaScript/TypeScript.
...
- **FR-013**: O harness MUST reconhecer `EscutIA/platform/site/` como uma aplicação Next.js full-stack separada, contendo o frontend e as rotas da API; arquivos desse contexto MUST NOT ser usados na preflight, como artefatos de treinamento ou como evidência de prontidão, e uma solicitação que envolva ambos os contextos MUST ter escopos e validações separados.
- **FR-014**: A arquitetura planejada da plataforma MUST usar JavaScript/TypeScript no runtime Node.js do Next.js para a API e a inferência; `testar_modelo_lora.py` MUST NOT fazer parte do fluxo da aplicação. A escolha do runtime e do formato compatível do modelo será definida antes da implementação.
...
- **EscutIA Platform Context**: aplicação Next.js full-stack separada, composta pelo site e pelas rotas da API, fora do fluxo de fine-tuning.
...
- **SC-010**: Uma auditoria da documentação identifica `EscutIA/platform/site/` como o único projeto da aplicação, registra suas rotas API server-side, indica runtime Node.js e não referencia `testar_modelo_lora.py` como parte da execução.

## Assumptions

- O projeto correto é a pasta existente `EscutIA/` no diretório pai do harness.
- A aplicação do produto fica em `EscutIA/platform/site/`, com frontend e API no mesmo projeto Next.js; ela não substitui nem altera a fonte estável de fine-tuning.

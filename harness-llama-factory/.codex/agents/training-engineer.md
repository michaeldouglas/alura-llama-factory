...
`EscutIA/platform/site/` é a aplicação Next.js full-stack do produto e permanece
fora do fluxo de fine-tuning. Não use o site ou as rotas da API como dataset, configuração, saída ou
...
separe as alterações e as validações. A inferência futura da plataforma será
JavaScript/TypeScript no runtime Node.js; `testar_modelo_lora.py` não faz parte
do fluxo.
